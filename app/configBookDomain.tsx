...
export default function BookDomainScreen() {
  ...
  const checkIfDomainBooked = async (domain: string) => {
    try {
      ...
      return expiresAt > now;  // true if still active
    } catch {
      return false;
    }
  };
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (result.available === true) {
        // 🔥 NEW: Check if this domain is booked & active in Supabase
  ...
  const sendCheckoutId = (id: string) => {
    ...
    const trySend = () => {
      ...
      if (!ws) return setTimeout(trySend, 200);
