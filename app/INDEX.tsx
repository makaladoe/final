...
// ⭐ NEW: import Supabase notifications service
import { getUserNotifications } from "@/services/supabaseService";
...
export default function HomeScreen() {
  ...
  // ⭐ Load unread notifications from Supabase
  useEffect(() => {
    const loadUnread = async () => {
      try {
        if (!firebaseUser?.uid) return;

        const { data, error } = await getUserNotifications(firebaseUser.uid);
        if (error) {
        ...
        console.log("Unread load error:", err);
      }
    };
  ...
  const actions: {
  ...
  }[] = [
    {
      ...
      icon: <MaterialCommunityIcons name="transfer" size={40} color="#fff" />,
    },
   {
      ...
      icon: <MaterialIcons name="dns" size={40} color="#fff" />,
    }, 
    {
      title: "Community",
      ...
      icon: <MaterialIcons name="group" size={40} color="#fff" />,
    },
    {
      ...
      route: "/delete",
      color: "#e53935",
      icon: <MaterialIcons name="lightbulb-outline" size={40} color="#fff" />,
    },
  ];
      ...
      >
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
          <View style={styles.profileHeader}>
            ...
            <Text style={styles.profileName}>{profileReady ? fullName : "Loading..."}</Text>
        ...
        >
          ...
          <View style={styles.headerRow}>
            ...
            <View style={{ marginLeft: 10, flex: 1 }}>
              ...
              <Text style={styles.nameText}>{profileReady ? fullName : "Loading..."}</Text>
            </View>
          ...
          <View style={styles.searchCardRow}>
            <TouchableOpacity
              ...
              activeOpacity={0.85}
              onPress={() => {
                if (!selectedCategory) return showCategoryWarning();
                router.push({
                  pathname: "/aiSuggester",
            ...
            <View style={styles.gridContainer}>
              {actions.map((a) => (
                <TouchableOpacity
                  key={a.title}
                  activeOpacity={0.86}
                  style={[styles.cardItem, { backgroundColor: a.color }]}
                  onPress={() => router.push(a.route)}
                >
                  {a.icon}
                  <Text style={styles.cardTitleWhite}>{a.title}</Text>
                  <Text style={styles.cardSubtitleWhite}>{a.subtitle}</Text>
                </TouchableOpacity>
              ))}
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 17, textAlign: "center", fontWeight: "600", color: theme.colors.black, marginVertical: 6 },
  ...
  searchCardLabel: { marginTop: 10, fontSize: 14, fontWeight: "600", color: "#000", textAlign: "center" },

  gridSection: { marginTop: 40 },
  sectionHeader: { fontSize: 18, fontWeight: "800", color: theme.colors.black, marginBottom: 12 },
  ...
  cardTitleWhite: { marginTop: 10, fontSize: 16, fontWeight: "700", color: "#fff", textAlign: "center" },
  cardSubtitleWhite: { fontSize: 12, color: "#fff", textAlign: "center", marginTop: 4 },

  overlay: { ...StyleSheet.absoluteFillObject, backgroundColor: "rgba(0,0,0,0.6)" },
  profilePanel: { position: "absolute", top: 0, left: 0, width: "78%", height: "100%", backgroundColor: "#fff", elevation: 12, borderTopRightRadius: 16, borderBottomRightRadius: 16, padding: 22 },
  profileHeader: { alignItems: "center", marginTop: 40 },
  avatarLarge: { width: 90, height: 90, borderRadius: 45, borderWidth: 3, borderColor: theme.colors.secondary, justifyContent: "center", alignItems: "center" },
  ...
  contactSection: { marginTop: 35, paddingTop: 18, borderTopWidth: 1, borderTopColor: "#ddd" },
  contactTitle: { fontSize: 18, fontWeight: "800", color: theme.colors.primary, marginBottom: 12, textAlign: "center" },
  ...
  categorySelector: { backgroundColor: "white", paddingVertical: 14, paddingHorizontal: 16, borderRadius: 10, borderWidth: 1, borderColor: "#999", flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  categorySelectorText: { fontSize: 15, color: "black", fontWeight: "600" },
  dropdownContent: { flexDirection: "row", flexWrap: "wrap", paddingVertical: 12, paddingHorizontal: 4 },
  categoryPill: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 20, borderWidth: 1.4, borderColor: "#333", backgroundColor: "white", margin: 5 },
  categoryPillText: { color: "#000", fontSize: 14, fontWeight: "600" },
});
