...
export default function DomainAuction() {
  ...
  const fetchAuctions = async () => {
    try {
      ...
      auctionsData.forEach(async (auction) => {
        ...
        formattedBids.sort((a: Bid, b: Bid) => new Date(b.placedAt).getTime() - new Date(a.placedAt).getTime());
        setAllBids((prev) => ({ ...prev, [auction.domain]: formattedBids }));

        if (formattedBids.length > 0) {
          const highest = formattedBids.reduce((acc: Bid, b: Bid) => (b.amount > acc.amount ? b : acc), formattedBids[0]);
          setAuctions((prev) =>
            prev.map((a) =>
              a.domain === auction.domain
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      Object.keys(animInstances).forEach((k) => {
        try {
          animInstances[k]?.stop();
        } catch {}
      });
    };
  }, [user]);
  ...
  const handleBid = async () => {
    ...
    try {
      ...
      if (svcError) {
        const errorMessage =
          (svcError as any)?.message ??
          (svcError as any)?.error_description ??
      ...
      if (hadPrevious) {
        Alert.alert("Success", `Your bid has been updated to Ksh${bidValue}`);
      } else {
      ...
      const friendly = (err as any)?.message ?? "Failed to place bid. Please try again.";
  ...
  const startMarquee = (domain: string, expired: boolean) => {
    ...
    try {
      animInstances[domain]?.stop();
    } catch {}

    scrollAnimRefs[domain].setValue(SCREEN_WIDTH + 20); // start offscreen with buffer
    const distance = SCREEN_WIDTH + textWidth + 40; // extra buffer so text fully visible
    const duration = expired
      ? Math.max(15000, distance * 10) // slower for expired
      : Math.max(8000, distance * 12);

    const anim = Animated.loop(
      Animated.timing(scrollAnimRefs[domain], {
        toValue: -textWidth - 20, // end offscreen
        duration,
  ...
  /** Update marquees when bids/auctions change */
  useEffect(() => {
    auctions.forEach((item) => {
      ...
      // EXPIRED: only show expired + highest bid amount
      if (status === "Expired") {
      ...
      if (textWidths[item.domain]) {
        setTimeout(() => startMarquee(item.domain, status === "Expired"), 50);
      }
    });
