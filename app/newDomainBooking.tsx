...
// ⭐ Material Icons (top-right bookings icon)
...
const STK_API_KEY = extra.STK_API_KEY;
...
const withTimeout = (p: Promise<Response>, ms = 8000): Promise<Response> =>
...
const safeFetchJson = async <T = any>(url: string): Promise<T | null> => {
  try {
    const res: Response = await withTimeout(fetch(url), 10000);
      ...
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          ...
          try {
            ...
            if (STK_API_KEY) {
              headers["Authorization"] = `Bearer ${STK_API_KEY}`;
            }

            const response = await fetch(STK_API_URL, {
              method: "POST",
              headers,
              body: JSON.stringify({
            ...
            if (data.ResponseCode === "0") {
              // SAVE BOOKING
              const { data: savedData, error } = await saveDomainBooking({
                user_id: user.uid,
                full_name: fullName,
                phone,
                email,
              ...
              Alert.alert("Payment Failed", JSON.stringify(data));
            }
          } catch (err) {
            console.error(err);
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <Text style={styles.lead}>
        Secure your perfect .KE domain in seconds.
      </Text>
        ...
        <View style={styles.tldPicker}>
          {TLDS.map((t) => (
            <Pressable
              ...
              style={[
                styles.tldOption,
                selectedTld === t && styles.tldActive,
              ]}
            >
              <Text
                style={[
                  styles.tldText,
                  selectedTld === t && styles.tldTextActive,
                ]}
              >
                {t}
              </Text>
          ...
          >
            {loading ? (
              ...
              <Text style={styles.payBtnText}>
                Pay KES {RATE_PER_WEEK} with M-PESA
              </Text>
            )}
          </TouchableOpacity>
        </View>
...
const styles = StyleSheet.create({
  ...
  checking: { color: theme.colors.gray600, marginTop: 8 },
